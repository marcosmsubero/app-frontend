...
function DayEventCard({ event, canManage = false, onEdit, onDelete }) {
  ...
  const hasMap = Number.isFinite(Number(event?.latitude)) && Number.isFinite(Number(event?.longitude));
      ...
      <div className="discoverEventFlipCard__inner">
        <div className="discoverEventFlipCard__face discoverEventFlipCard__face--front">
          <div className="discoverEventFlipCard__mediaWrap">
            <button
              ...
              aria-label={`Ver detalles de ${eventTitle(event)}`}
            >
              {imageSrc ? (
                <img
                  src={imageSrc}
          ...
          <div className="discoverEventFlipCard__frontBody">
            <h3 className="discoverEventFlipCard__title">{eventTitle(event)}</h3>
          </div>
        </div>
        ...
        <div className="discoverEventFlipCard__face discoverEventFlipCard__face--back">
          ...
          <div className="discoverEventFlipCard__details">
            ...
            <p className="discoverEventFlipCard__detailLine">{event.meeting_point || "Sin dirección"}</p>
          ...
          <div className="discoverEventFlipCard__footer discoverEventFlipCard__footer--actions">
            <span className="discoverEventFlipCard__host">
              <CreatorLink event={event} />
            </span>

            <div className="discoverEventFlipCard__actions">
              {hasMap ? (
                <button
                  type="button"
                  className="discoverInlineLink discoverInlineLink--button"
                  onClick={() =>
                    openInMaps(event.latitude, event.longitude, event.meeting_point || eventTitle(event))
                  }
                >
                  Abrir ubicación
                </button>
              ...
              {canManage ? (
                <>
                  ...
                  <button
                    type="button"
                    className="discoverInlineLink discoverInlineLink--button danger"
...
function ProfileAgenda({ meetups = [], canCreate = false, onCreateEvent, onEditEvent, onDeleteEvent }) {
...
export default function ProfilePage() {
  ...
  function handleEditEvent(event) {
    navigate(`/crear-evento?edit=${encodeURIComponent(event.id)}&day=${encodeURIComponent(localDayKey(event.starts_at))}`);
  }
        ...
        >
          <div className="profileIdentityMain">
            ...
            <div className="profileIdentityCopy">
              ...
              <div className="profileLocationRow">
                <span className="profileLocationText">
                  {formatLocation(profileData.location)}
                </span>
                {profileData.location_verified ? (
                  <span className="profileVerifiedBadge">Ubicación verificada</span>
                ) : null}
              </div>
              {!isPublicProfile && !profileData.location_verified ? (
                <Link to="/onboarding?mode=edit" className="profileVerifyLink">
                  Verificar ubicación
                </Link>
              ) : null}
            </div>
